...
| 高光 | -100 ~ +100 | 使用平滑遮罩调整高光区域 |
| 阴影 | -100 ~ +100 | 使用平滑遮罩调整阴影区域 |
| 白色色阶 | -100 ~ +100 | 调整图像最亮端的极限值 |
| 黑色色阶 | -100 ~ +100 | 调整图像最暗端的极限值 |

**算法特点：**
- 使用 `smoothstep` 函数创建平滑的亮度遮罩，避免硬边缘
- 高光遮罩范围：亮度 0.35-1.0（平滑过渡）
- 阴影遮罩范围：亮度 0.0-0.65（平滑过渡）
...
**算法特点：**
- 色调调整使用精确的 RGB 通道混合（参考 Lightroom 实现）
- 放大效果系数确保参数范围符合用户预期
- 洋红：增强红+蓝通道，减弱绿通道
- 荧光绿：增强绿通道，创造自然的绿色调
...
**鲜艳度算法特点：**
- 转换到 HSV 颜色空间进行处理
- 使用高斯权重 centered around mid-tones (mean=0.45)
- 肤色保护：对肤色区域减少调整强度
- 低饱和度区域获得更强的调整
...
本项目参考了专业图像处理软件（如 Adobe Lightroom）的实现方式：

### 色调调整算法
- 使用 `smoothstep` 函数创建平滑的亮度遮罩
- 三次插值确保过渡自然，避免色调分离
- 亮度比例计算保持色彩平衡

### 鲜艳度算法
- HSV 颜色空间处理
- 高斯权重函数：低饱和度和中间调区域获得更强调整
- 肤色保护：减少对肤色区域的影响

### 白平衡算法
- 色温：红/蓝通道比例调整
- 色调：精确的 RGB 通道混合模拟 Lab 颜色空间调整
